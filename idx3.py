        ...
        if filename.endswith(".csv"):
            ...
            for encoding in encodings_to_try:
                try:
                    ...
                    print(f"[{filename}] {encoding} 시도 중 오류 발생: {e}")
...
for i in range(len(data_scaled) - SEQ_LENGTH):
    ...
    y_list.append(data_scaled[i + SEQ_LENGTH][0])
...
# 5️⃣ 미래 예측 (추가된 코드)
# -----------------------------
def predict_future(model, last_sequence, scaler, days_to_predict):
    predicted_list = []
    # detach()를 사용해 기존 그래프에서 분리하여 복사
    current_sequence = last_sequence.clone().detach()

    for _ in range(days_to_predict):
        # 예측
        with torch.no_grad():
            prediction = model(current_sequence.unsqueeze(0)).numpy()
        
        # 예측값을 역정규화
        predicted_price = scaler.inverse_transform(
            np.concatenate([prediction, np.zeros((1, 5))], axis=1)
        )[0][0]
        predicted_list.append(predicted_price)
        
        # 다음 예측을 위한 시퀀스에 예측값 추가
        scaled_pred_row = scaler.transform(np.array([predicted_price, 0, 0, 0, 0, 0]).reshape(1, -1))
        
        current_sequence = torch.cat(
            (current_sequence[1:], torch.tensor(scaled_pred_row[:,0:1].reshape(1,1), dtype=torch.float32)), 0
        )
        
    return predicted_list

DAYS_TO_PREDICT = 30  # 예측하고 싶은 미래 일수
last_sequence = X[-1] # 학습 데이터의 마지막 시퀀스
...
plt.plot(range(len(pred_prices)), pred_prices, label='Predicted Price')

# 미래 예측 데이터를 기존 그래프에 이어서 그리기
future_indices = range(len(actual_prices), len(actual_prices) + DAYS_TO_PREDICT)
plt.plot(future_indices, future_predictions, label=f'Future Forecast ({DAYS_TO_PREDICT} days)', color='red', linestyle='--')
